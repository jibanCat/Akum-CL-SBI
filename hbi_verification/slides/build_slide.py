...
<div class="deck">
  ...
  <div class="body3">
    ...
    <div class="card green">
      ...
      <p>The integral is expensive &hellip; <i>unless</i> we already have stored chains $\theta_j^{(s)}\!\sim\!p(\theta_j\!\mid\!d_j)$ from per-cluster fits (with training prior $\pi_0$). The data likelihood $p(d_j\!\mid\!\theta_j)$ is <i>baked into</i> which samples appear; we just reweight the original prior $\pi_0$ to the population prior:</p>
      <div class="eq">$$p(\Lambda\mid \mathrm{data})\;\propto\;\pi(\Lambda)\,\prod_j\,\tfrac1S\!\sum_s\!\tfrac{p(\theta_j^{(s)}\mid\Lambda)}{\pi_0(\theta_j^{(s)})}\,,\quad\theta_j^{(s)}\!\sim\!p(\theta_j\!\mid\!d_j)$$</div>
